...
var _rotationMatrix;
var _raycaster;
var CameraControls = class CameraControls extends EventDispatcher {
	/**
	...
	constructor(camera, domElement) {
		...
		const onPointerMove = (event) => {
			...
			if (event.pointerType === "touch") switch (this._activePointers.length) {
				...
				case 2:
					...
					break;
				case 3: this._state = this.touches.three;
			}
			else {
				if (!this._isDragging && this._lockedPointer || this._isDragging && (event.buttons & MOUSE_BUTTON.LEFT) === MOUSE_BUTTON.LEFT) this._state = this._state | this.mouseButtons.left;
				if (this._isDragging && (event.buttons & MOUSE_BUTTON.MIDDLE) === MOUSE_BUTTON.MIDDLE) this._state = this._state | this.mouseButtons.middle;
				if (this._isDragging && (event.buttons & MOUSE_BUTTON.RIGHT) === MOUSE_BUTTON.RIGHT) this._state = this._state | this.mouseButtons.right;
			}
			dragging();
		};
		...
		const onMouseWheel = (event) => {
			...
			if (this.dollyToCursor || this.mouseButtons.wheel === ACTION.ROTATE || this.mouseButtons.wheel === ACTION.TRUCK) {
				...
				if (lastScrollTimeStamp - now < 1e3) this._getClientRect(this._elementRect);
			...
			switch (event.ctrlKey ? ACTION.ZOOM : this.mouseButtons.wheel) {
				case ACTION.ROTATE:
		...
		const startDragging = (event) => {
			...
			if (this._activePointers.length >= 2) {
				...
				const y = (this._activePointers[0].clientY + this._activePointers[1].clientY) * .5;
			...
			if (!event) {
				...
				if (!this._lockedPointer && (event.buttons & MOUSE_BUTTON.LEFT) === MOUSE_BUTTON.LEFT) this._state = this._state | this.mouseButtons.left;
		...
		const dragging = () => {
			...
			extractClientCoordFromEvent(this._activePointers, _v2);
			const lockedPointer = this._domElement && this._domElement.ownerDocument.pointerLockElement === this._domElement ? this._lockedPointer || this._activePointers[0] : null;
		...
		const onPointerLockChange = () => {
			if (!(this._domElement && this._domElement.ownerDocument.pointerLockElement === this._domElement)) this.unlockPointer();
		};
	...
	*/
	...
	_dollyToNoClamp(distance, enableTransition = false) {
		...
		if (this.colliderMeshes.length >= 1) {
			...
			if (!(lastRadius > distance) && isCollided) return Promise.resolve();
	...
	*/
	lookInDirectionOf(x, y, z, enableTransition = false) {
		const position = _v3A.set(x, y, z).sub(this._targetEnd).normalize().multiplyScalar(-this._sphericalEnd.radius).add(this._targetEnd);
	...
	*/
	fitToSphere(sphereOrMesh, enableTransition) {
		const promises = [];
		const boundingSphere = "isObject3D" in sphereOrMesh ? CameraControls.createBoundingSphere(sphereOrMesh, _sphere) : _sphere.copy(sphereOrMesh);
	...
	*/
	lerp(stateA, stateB, t, enableTransition = false) {
		...
		if ("spherical" in stateA) _sphericalA.set(...stateA.spherical);
		else {
			const positionA = _v3B.set(...stateA.position);
			_sphericalA.setFromVector3(positionA.sub(targetA).applyQuaternion(this._yAxisUpSpace));
		}
		const targetB = _v3C.set(...stateB.target);
		if ("spherical" in stateB) _sphericalB.set(...stateB.spherical);
		else {
	...
	*/
	getDistanceToFitBox(width, height, depth, cover = false) {
		...
		return ((cover ? boundingRectAspect > aspect : boundingRectAspect < aspect) ? height : width / aspect) * .5 / Math.tan(fov * .5) + depth * .5;
	}
	/**
	...
	*/
	getDistanceToFitSphere(radius) {
		...
		const hFOV = Math.atan(Math.tan(vFOV * .5) * this._camera.aspect) * 2;
		const fov = 1 < this._camera.aspect ? vFOV : hFOV;
		return radius / Math.sin(fov * .5);
	}
	/**
	...
	*/
	getTarget(out, receiveEndValue = true) {
		return (!!out && out.isVector3 ? out : new THREE.Vector3()).copy(receiveEndValue ? this._targetEnd : this._target);
	}
	/**
	...
	*/
	getPosition(out, receiveEndValue = true) {
		return (!!out && out.isVector3 ? out : new THREE.Vector3()).setFromSpherical(receiveEndValue ? this._sphericalEnd : this._spherical).applyQuaternion(this._yAxisUpSpaceInverse).add(receiveEndValue ? this._targetEnd : this._target);
	}
	/**
	...
	*/
	getSpherical(out, receiveEndValue = true) {
		return (out || new THREE.Spherical()).copy(receiveEndValue ? this._sphericalEnd : this._spherical);
	}
	/**
	...
	*/
	getFocalOffset(out, receiveEndValue = true) {
		return (!!out && out.isVector3 ? out : new THREE.Vector3()).copy(receiveEndValue ? this._focalOffsetEnd : this._focalOffset);
	}
	/**
	...
	update(delta) {
		...
		if (this.dollyToCursor) {
			if (isPerspectiveCamera(this._camera) && this._changedDolly !== 0) {
				...
				const worldToScreen = this._sphericalEnd.radius * Math.tan(camera.getEffectiveFOV() * DEG2RAD * .5);
				const lerpRatio = (this._sphericalEnd.radius - dollyControlAmount - this._sphericalEnd.radius) / this._sphericalEnd.radius;
				...
				const lerpRatio = -(this._zoom - dollyControlAmount - this._zoom) / this._zoom;
				const cameraDirection = this._getCameraDirection(_cameraDirection);
		...
		if (!approxZero(this._focalOffset.x) || !approxZero(this._focalOffset.y) || !approxZero(this._focalOffset.z)) {
		...
		} else if (!updated && this._updatedLastTime) this.dispatchEvent({ type: "sleep" });
		this._lastDistance = this._spherical.radius;
		this._lastZoom = this._zoom;
	...
	*/
	...
	_encloseToBoundary(position, offset, friction) {
		...
		const deltaClampedTarget = this._boundary.clampPoint(newTarget, _v3C).sub(newTarget);
	...
	_updateNearPlaneCorners() {
		if (isPerspectiveCamera(this._camera)) {
			...
			const heightHalf = Math.tan(fov * .5) * near;
	...
	_truckInternal = (deltaX, deltaY, dragToOffset, screenSpacePanning) => {
		...
		if (isPerspectiveCamera(this._camera)) {
			...
			const fov = this._camera.getEffectiveFOV() * DEG2RAD;
			const targetDistance = offset.length() * Math.tan(fov * .5);
	...
	_dollyInternal = (delta, x, y) => {
		const dollyScale = Math.pow(.95, -delta * this.dollySpeed);
	...
	_zoomInternal = (delta, x, y) => {
		const zoomScale = Math.pow(.95, delta * this.dollySpeed);
	...
	_collisionTest() {
		...
		if (!(this.colliderMeshes.length >= 1)) return distance;
